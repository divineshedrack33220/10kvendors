...
exports.createOrder = async (req, res) => {
  try {
    ...
    const deliveryFee = 5000; // Matches checkout.html
    const total = subtotal + deliveryFee;

    // Create order
    const order = new Order({
      user: user._id,
      addressId: deliveryAddress, // Map deliveryAddress to addressId
      ...
      tracking: [{ status: paymentMethod === 'Pay on Delivery' ? 'Pending' : 'Awaiting Payment' }],
      paymentStatus: paymentMethod === 'Pay on Delivery' ? 'pending' : 'awaiting_payment',
    ...
    // Populate user and address for WebSocket emission
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    if (!order.addressId || !mongoose.isValidObjectId(order.addressId)) {
      ...
      if (!userAddress) {
        throw new Error('Cannot update order: no valid address found for user');
      }
...
exports.getMetrics = async (req, res) => {
  try {
    ...
    const netRevenue = totalRevenue * 0.9; // Example: 10% fees
    ...
    const chargebackRate = 0.5; // Placeholder
